...
const amplifyConfig = {
  ...
  amplifyConfig.oauth.redirectSignIn = `${expoScheme}signin/`;
  amplifyConfig.oauth.redirectSignOut = `${expoScheme}signout/`;
  // console.log(redirectUrl);
    ...
    return (dispatch) => {
      Auth.currentAuthenticatedUser({ bypassCache: true })
        .then((user) => {
          ...
          if (user.attributes && user.attributes.preferred_username && user.attributes.preferred_username.length > 0) {
            dispatch(getUser());
          } else {
            dispatch(getFBUser());
          }
